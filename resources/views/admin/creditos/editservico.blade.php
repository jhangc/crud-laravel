    ...
    <div class="col-md-12">
        <div class="card card-outline card-primary">
            <div class="card-header">
                <h3 class="card-title">Editar Credito Servicio</h3>
            </div>
            <div class="card-body">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function cargardata() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    llenarBoletas(response.boletas);
                },
                error: function(xhr) {
        ...
        function llenarVentasDiarias(ventas) {
            ventasDiarias = ventas.map(venta => ({
                dia: venta.dia,
                min: venta.cantidad_minima,
                max: venta.cantidad_maxima,
                promedio: parseFloat(venta.promedio)
            }));
            actualizarTablaVenta();
        }

        function llenarDeudasFinancieras(deudas) {
            deudasFinancierasArray = deudas.map(deuda => ({
        ...
        function llenarGastosFamiliares(gastos) {
            inventarioArray1 = gastos.map(gasto => ({
                descripcion: gasto.descripcion,
                precioUnitario: parseFloat(gasto.precio_unitario),
                cantidad: parseFloat(gasto.cantidad),
                montoTotal: parseFloat(gasto.precio_unitario * gasto.cantidad).toFixed(2)
            }));
        ...
        function llenarGarantia(garantia) {
            if (garantia) {
                ...
                $('#archivo_garantia').val(garantia.documento_pdf);
            }
        }

        function llenarBoletas(boletas) {
            boletasArray = boletas.map(boleta => ({
                numeroBoleta: boleta.numero_boleta,
                montoBoleta: parseFloat(boleta.monto_boleta),
                descuentoBoleta: parseFloat(boleta.descuento_boleta),
                totalBoleta: parseFloat(boleta.total_boleta)
            }));
